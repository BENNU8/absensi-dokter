<?php
session_start();
if(isset($_SESSION['dokter'])) header("Location: dashboard.php");
?>
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<title>Login Dokter | Best Clinic</title>
...
body{
  margin:0;
  min-height:100vh;
  background:linear-gradient(120deg,#0f5132,#198754,#36c48c);
...
button{
  ...
  cursor:pointer;
  color:#fff;
  background:linear-gradient(135deg,#198754,#36c48c);
  transition:.3s;
}
button:hover{
...
<div class="card">
  ...
  <h2>LOGIN DOKTER</h2>
  <p>Best Clinic – Sistem Absensi Digital</p>

  <form method="POST" action="../proses/proses_login.php">
    <input name="username" placeholder="Username Dokter" required>
